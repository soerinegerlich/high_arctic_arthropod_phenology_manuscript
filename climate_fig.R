...
Temp <- ggplot(data=dfair4, aes(Year, Summer))+
  ...
  geom_point(data = dfair4, aes(x=Year, y=Spring), size = 2, color = "black")+
  geom_line(data = dfair4, aes(x=Year, y=Spring), size = 1, color = "black")+
  geom_smooth(data = dfair4, aes(x=Year, y=Spring),method = "lm", size = 1.5, linetype = "dashed", color = "black", se = FALSE)+
  ylab("Air temperature (°C)")+
  xlab("")+
  ylim(-16,16)+
  theme_bw()+
  annotate("text", x = 2006, y = 10, label = "Summer temperature", size = 5)+
  annotate("text", x = 2006, y = -3, label = "Spring temperature", size = 5)+
...
Snow <- ggplot(data=dfsnowmelt_climatestation, aes(x=Year, y=DOY)) +
  ...
  theme_bw()+
  annotate("text", x = 2004, y = 195, label = "Snowmelt timing", size = 5)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
